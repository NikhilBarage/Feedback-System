  ...
  <div class="card-header py-3">
    <h6 class="m-0 font-weight-bold text-primary">
		...
		<form action="page1.php" method="get">
			<?php
				echo "<input type='hidden' name='adding' value='teacher'>";
			?>
    ...
    ?>
        ...
        <?php
            if(isset($_GET['semname'])){
				...
				echo "<input type='hidden' name='typee' value='teach'>";
            }
        ?>
	...
	<?php
                if (mysqli_num_rows($result) > 0) {
                    while ($teachers_row = mysqli_fetch_assoc($result)) {
                        echo "<tr>
                                <td><input type='checkbox' name='id[]' value='".$teachers_row['id']."'></td>
                                <td>".$teachers_row['id']."</td>
                                <td>".$teachers_row['name']."</td>
                                <td>".$teachers_row['email']."</td>
                                <td>".$teachers_row['username']."</td>
                              </tr>";
                    }
                } else {
